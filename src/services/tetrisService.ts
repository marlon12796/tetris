...
import { doesCollideBottom, doesCollideLeft, doesCollideRight } from './collisionService';

export const start = (state: TetrisState) => {
...
export const moveDown = (state: TetrisState) => {
	update(state);
};

export const moveLeft = (state: TetrisState) => {
...
export const moveRight = (state: TetrisState) => {
	if (state.locked || !state.currentPiece) {
		return;
	}
	state.setCurrentPiece(state.currentPiece.store());
...
export const rotate = (state: TetrisState) => {
	if (state.locked || !state.currentPiece) {
		return;
	}
...
export const drop = (state: TetrisState) => {
	if (state.locked || !state.currentPiece) return;
...
export const update = (state: TetrisState) => {
	if (state.locked || !state.currentPiece) return;
	...
	if (doesCollideBottom(state)) {
		...
		if (isGameOver(state)) onGameOver(state);
		return;
	}
...
export const reset = (state: TetrisState) => {
	if (state.status === GameStatus.Started || state.status === GameStatus.Paused) {
		state.setIsResetting(true);
		createInitialState(state);
	}
};

export const pause = (state: TetrisState) => {
...
export const resume = (state: TetrisState) => {
...
export const clearFullLines = (state: TetrisState) => {
	const newMatrix = JSON.parse(JSON.stringify(state.matrix));
	let numberOfFullLines = 0;
	for (let rowIndex = MatrixUtil.HEIGHT - 1; rowIndex >= 0; rowIndex--) {
		const row = newMatrix.slice(rowIndex * MatrixUtil.WIDTH, (rowIndex + 1) * MatrixUtil.WIDTH);
		const isRowFullySolid = row.every((cell: Tile) => cell.isSolid);
		if (isRowFullySolid) {
			numberOfFullLines++;
			const topPortion = newMatrix.slice(0, rowIndex * MatrixUtil.WIDTH);
			newMatrix.splice(0, ++rowIndex * MatrixUtil.WIDTH, ...MatrixUtil.EmptyRow.concat(topPortion));
		}
	}
	state.setMatrix(newMatrix);
	state.setCurrentPiece(state.nextPiece);
...
const setPointsAndSpeed = (state: TetrisState, numberOfClearedLines: number) => {
	if (numberOfClearedLines === 0) return;

	const newClearedLines = state.numberOfClearedLines + numberOfClearedLines;
	const addedPoints = MatrixUtil.POINTS[numberOfClearedLines - 1] ?? MatrixUtil.POINTS?.at(-1);

	const newSpeed = (state.initSpeed +
		Math.max(state.initSpeed + Math.floor(newClearedLines / (MatrixUtil.HEIGHT / 2)), 6)) as Speed;
	state.setPoint(addedPoints);
	state.setSpeed(newSpeed);
	state.setNumberOfClearedLines(newClearedLines);
};

const isGameOver = (state: TetrisState) => {
	return state.matrix.slice(0, MatrixUtil.WIDTH).some((cell) => cell.isSolid);
};

const onGameOver = (state: TetrisState) => {
	state.setIsResetting(true);
	createInitialState(state);
	AudioService.gameOver(state.isSoundOn);
};

const createInitialState = (state: TetrisState) => {
