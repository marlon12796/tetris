...
export const Board: FunctionComponent = () => {
	...
	const resettingAnimation = useCallback(() => {
		...
		for (let i = 0; i <= 40; i++) {
			setTimeout(() => {
				if (i <= 19) {
					newOverState.splice((19 - i) * MatrixUtil.WIDTH, MatrixUtil.WIDTH, ...MatrixUtil.FullRow);
					setOverState([...newOverState]);
				} else if (i >= 20 && i <= 39) {
					newOverState.splice((i - 20) * MatrixUtil.WIDTH, MatrixUtil.WIDTH, ...MatrixUtil.EmptyRow);
					setOverState(() => {
						return [...newOverState];
					});
				} else {
	...
	return (
		<StyledBoard className='game-board'>
			{board.map(({ isFilled, isSolid, isAnimated }, index) => {
				const shouldCellBeAnimated = animatingRows?.some(({ min, max }) => index >= min && index <= max);
				return (
					<Tile
						key={index}
						isAnimated={(isAnimating && shouldCellBeAnimated) || (isAnimated && !isClearingFullRows)}
						isFilled={isFilled}
						isSolid={isSolid}
					/>
				);
